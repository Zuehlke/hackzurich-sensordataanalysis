...
object CassandraCQL {
  ...
  def createSchema(s: Session): Unit = {
    ...
    s.execute("CREATE TABLE IF NOT EXISTS sensordata.barometer( date timestamp, deviceid text, relativealtitude double, pressure double, PRIMARY KEY (deviceid, date) ) WITH CLUSTERING ORDER BY (date DESC)")
